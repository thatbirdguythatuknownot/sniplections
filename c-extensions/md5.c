...
PyObject *empty_string_hexdigest; /* "d41d8cd98f00b204e9800998ecf8427e" */

PyObject *
md5_hexdigest(PyObject *Py_UNUSED(self), PyObject *string)
{
    u32 A = 0x67452301UL, B = 0xefcdab89UL, C = 0x98badcfeUL, D = 0x10325476UL, curlen = 0;
    Py_ssize_t size_string, n, length = 0;
    const u8 *buffer;
    u8 md5_buffer[MD5_BLOCKSIZE];
    if (likely(PyUnicode_Check(string))) {
        if (unlikely(PyUnicode_KIND(string) != PyUnicode_1BYTE_KIND)) {
            PyErr_SetString(PyExc_ValueError,
                            "md5.hexdigest() argument is not an ASCII string");
            return NULL;
        }
        size_string = PyUnicode_GET_LENGTH(string);
        buffer = (const u8 *)PyUnicode_DATA(string);
    }
    else if (PyBytes_Check(string)) {
        size_string = Py_SIZE(string);
        buffer = (const u8 *)PyBytes_AS_STRING(string);
    }
    else if (PyByteArray_Check(string)) {
        size_string = Py_SIZE(string);
        buffer = (const u8 *)_PyByteArray_CAST(string)->ob_start;
    }
    else {
        PyErr_Format(PyExc_TypeError,
                     "md5.hexdigest() expected 'str', 'bytes', or 'bytearray'; "
                     "got %.200s",
                     Py_TYPE(string)->tp_name);
        return NULL;
    }
    if (unlikely(size_string == 0)) {
        Py_INCREF(empty_string_hexdigest);
        return empty_string_hexdigest;
    }
    ...
    while (likely(size_string > 0)) {
        if (curlen == 0 && size_string >= MD5_BLOCKSIZE) {
            ...
            n = (Py_ssize_t)(MD5_BLOCKSIZE - curlen);
            if (size_string < n) {
                n = size_string;
            }
    ...
    STORE32(8,  C, out);
    STORE32(12, D, out);
    return _Py_strhex((const char *)out, MD5_DIGESTSIZE);
}

PyDoc_STRVAR(md5_hexdigest__doc__, "\
md5.hexdigest(string)\n\
    Return the md5 hash hexdigest of *string*.\n\
");

static struct PyMethodDef MD5_functions[] = {
    {"hexdigest", (PyCFunction)(void(*)(void))md5_hexdigest, METH_O, md5_hexdigest__doc__},
    {NULL}
};

static int
md5_traverse(PyObject *module, visitproc visit, void *arg)
{
    Py_VISIT(empty_string_hexdigest);
    return 0;
}

static int
md5_clear(PyObject *module)
{
    Py_CLEAR(empty_string_hexdigest);
    return 0;
}

static void
md5_free(void *module)
{
    md5_clear((PyObject *)module);
}
...
static struct PyModuleDef md5module = {
        ...
        .m_name = "md5",
        .m_size = SIZEOF_VOID_P,
        .m_methods = MD5_functions,
        .m_traverse = md5_traverse,
        .m_clear = md5_clear,
        .m_free = md5_free,
};
...
PyInit_md5(void)
{
    empty_string_hexdigest = PyUnicode_DecodeUTF8Stateful("d41d8cd98f00b204e9800998ecf8427e", 32, NULL, NULL);
    if (unlikely(empty_string_hexdigest == NULL)) {
        return NULL;
    }
    return PyModuleDef_Init(&md5module);
}
